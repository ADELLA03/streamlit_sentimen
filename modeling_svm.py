...
def run_svm(train, test, vectorizer, use_smote=False, kernel="linear", C=1.0, gamma="scale"):
    ...
    accuracy = svm.score(X_tfidf_test, y_test)
    ...
    conf_path = os.path.join(RESULT_DIR, "conf_matrix_svm.png")
    ...
    report_path = os.path.join(RESULT_DIR, "report_svm.txt")
    with open(report_path, "w", encoding="utf-8") as f: f.write(report)
    pred_path = os.path.join(RESULT_DIR, "pred_svm.csv")
    pd.DataFrame({"y_true": y_test, "y_pred": y_pred}).to_csv(pred_path, index=False)
    model_path = os.path.join(RESULT_DIR, "model_svm.joblib")
